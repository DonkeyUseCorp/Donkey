...
public enum MediaCutEngine {
    ...
    private static func runSync(_ request: HarnessMediaCutRequest) -> HarnessMediaCutResult {
        ...
        guard let duration = probeDuration(ffprobe, inputURL), duration > 0 else {
            return failed("could not read the file's duration — is \(inputURL.lastPathComponent) a real media file?")
        }
        let streams = probeStreams(ffprobe, inputURL)
        ...
        let renderArgs = renderArguments(
            ...
            script: scriptURL,
            streams: streams,
            videoBitrate: streams.hasVideo ? probeVideoBitrate(ffprobe, inputURL) : nil
        )
        let render = runProcess(ffmpeg, renderArgs, timeout: max(180, min(duration * 2, 1800)))
        ...
        let outputDuration = probeDuration(ffprobe, outputURL) ?? 0
        return HarnessMediaCutResult(
    ...
    private static func probeDuration(_ ffprobe: URL, _ url: URL) -> Double? {
    ...
    private static func probeStreams(_ ffprobe: URL, _ url: URL) -> (hasVideo: Bool, hasAudio: Bool) {
        let result = runProcess(
            ffprobe,
    ...
    private static func probeVideoBitrate(_ ffprobe: URL, _ url: URL) -> String? {
        let result = runProcess(
            ffprobe,
    ...
    /// Default output path: `<name>-tightened.<ext>` beside the source, unless the caller named one.
    ...
    private static func runProcess(_ executable: URL, _ arguments: [String], timeout: TimeInterval) -> ProcessOutput {
        let process = Process()
        process.executableURL = executable
        process.arguments = arguments
        process.environment = DonkeyCommandBackends.shellEnvironment()
