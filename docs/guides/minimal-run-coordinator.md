...
Donkey also tracks the first screenshot segmentation model candidate for the off-the-shelf vision boundary. `OffTheShelfVisionModelCatalog` lists Ultralytics YOLO26 nano segmentation (`yolo26n-seg.pt`) as the current screenshot/crop segmentation candidate, with official docs rechecked on May 17, 2026. The runner boundary can convert injected local backend masks into traceable segmentation evidence, but live screenshot segmentation still requires an installed backend, crop pipeline integration, export/runtime choice, and measured latency before it can enter a guarded live loop.
...
Donkey also supports the first AI harness model registry and router boundary. `AIModelRegistry` stores model roles, providers, literal model ids, endpoints, capabilities, timeouts, prompt versions, eval status, docs URLs, and rollback ids as registry data. `AIModelRouter` routes slow-path jobs by job type, risk, privacy mode, latency tolerance, capabilities, failed-model history, and allowed providers. Privacy-sensitive planner routes prefer local Ollama-compatible entries when they are available. Privacy-sensitive voice-transcription routes prefer local runtime entries and currently select NVIDIA Parakeet TDT 0.6B v3, with Whisper large-v3-turbo as a local rollback candidate. `LocalVoiceTranscriptionAdapter` accepts bounded local audio buffers, routes to the selected local runtime model, returns transcript text plus timing/confidence trace metadata, and marks transcripts as inputs to the normal command parser rather than actions. The default transcription runtime is injectable and unavailable until a local NeMo/Whisper runtime is installed. Literal provider model ids should stay in registry entries, not planner/controller logic.

Donkey also supports the first measured screenshot segmentation runner boundary for the YOLO26 screenshot candidate. `YOLOScreenshotSegmentationRunner` accepts bounded crop requests, calls an injected local segmentation backend, converts masks into recorded off-the-shelf vision evidence, and preserves preprocessing/model/adapter latency in the same metadata codec used by the dry-run perception adapter. The default backend is unavailable; live segmentation requires an installed local runtime and target-specific benchmark evidence before it can affect live control.
...
This is a coordination, in-memory reflex trace, hot-loop contract, deterministic dry-run skeleton, bounded target-window frame-source, cheap metadata perception, recorded off-the-shelf local vision evidence, screenshot segmentation model-candidate catalog and runner boundary, deterministic controller, local-navigation dry-run, generic local-app task catalog/intent/live-runner boundary, local JSON/JSONL task-definition loading, local task-context intake, Accessibility control discovery/action execution boundary, document form-fill review and approval planning, sample task definitions across weather/media/document domains, guarded live-action smoke boundary, slow-planner sidecar, action-engine guardrail, latency-report, planner-hint contract, model-registry/router, local voice-transcription model selection and adapter boundary, OpenAI and Ollama-compatible planner adapters, memory, replay/eval, target-metadata, single-screenshot artifact, read-only Accessibility snapshot, and manual capture orchestration foundation only. It does not bundle live transcription or vision model runtimes, run OCR over captured pixels, provide the user-visible form-fill approval UI, provide a manual capture UI, persist high-volume reflex traces to disk, provide continuous streaming capture, or make model calls required for reflex ticks.
...
- Keep local voice transcription as a model-routed local runtime job. Transcription output is transcript text only and must enter the same validated command path as typed input before any task can run.
