...
Donkey also supports the first fast local-navigation dry-run slice. `LocalNavigationMetadataProjector` turns macOS window candidate metadata into typed local-navigation world state with frontmost/focused candidates, safe focus candidates, confidence, source age, and no raw pixels. `LocalNavigationControllerPolicy` can choose traceable dry-run semantic actions such as `focusWindow`, `activateCandidate`, `switchTab`, `wait`, or `observe` from that typed state. This slice is metadata-only and side-effect free; browser-tab metadata adapters, live input, and richer visual inference remain future work.
...
- Keep action-engine commands typed and replayable. Guardrails must run before live input and record permission, focus, rate-limit, hold-duration, release, and execution status.
...
- Keep local-navigation decisions typed and side-effect free until the guarded live-action slice. Metadata projectors may use window and Accessibility facts, but controllers should emit semantic actions only.
