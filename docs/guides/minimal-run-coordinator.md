...
- Keep deterministic task intents separate from planner hints. A supported local command should parse to `TaskIntent` from local app-task definitions without a model call; slow planner output may only help ambiguous parsing, app-knowledge lookup, or recovery after validation.
