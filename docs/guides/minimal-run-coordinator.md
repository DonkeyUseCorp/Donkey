...
Donkey also tracks the first screenshot segmentation model candidate for the off-the-shelf vision boundary. `OffTheShelfVisionModelCatalog` lists Ultralytics YOLO26 nano segmentation (`yolo26n-seg.pt`) as the current screenshot/crop segmentation candidate, with official docs rechecked on May 17, 2026. This is catalog metadata only; live screenshot segmentation still requires a local runner, crop pipeline, export/runtime choice, and measured latency before it can enter a guarded live loop.
...
This is a coordination, in-memory reflex trace, hot-loop contract, deterministic dry-run skeleton, bounded target-window frame-source, cheap metadata perception, recorded off-the-shelf local vision evidence, screenshot segmentation model-candidate catalog, deterministic controller, local-navigation dry-run, generic local-app task catalog/intent/live-runner boundary, sample task definitions across weather/media/document domains, guarded live-action smoke boundary, slow-planner sidecar, action-engine guardrail, latency-report, planner-hint contract, model-registry/router, local voice-transcription model selection, OpenAI and Ollama-compatible planner adapters, memory, replay/eval, target-metadata, single-screenshot artifact, read-only Accessibility snapshot, and manual capture orchestration foundation only. It does not run live transcription, live vision models, or OCR over captured pixels, perform Accessibility actions, provide robust UI control discovery, provide a manual capture UI, persist high-volume reflex traces to disk, provide continuous streaming capture, or make model calls required for reflex ticks.
...
- Keep live OS input disabled unless a caller explicitly configures live mode, allows input in policy, passes focus guard, and supplies an input backend. The macOS keyboard backend is narrow: it is for guarded local-app key/text commands, not arbitrary controller output.
...
- Keep local voice transcription as a model-routed local runtime job. The pointer prompt can capture microphone levels today, but speech-to-text still needs an adapter that consumes bounded audio buffers, calls the selected local runtime model, and emits validated transcript text before it enters command parsing.
