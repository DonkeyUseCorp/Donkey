...
- A stationary spawned cursor types compact progress text in its label. Clicking or focusing the label expands a smaller multiline follow-up input; while that input is focused or dirty, the cursor stays still until the user submits with Return or the send button, or collapses it with Escape.
- Holding Command while a spawned cursor is selected pauses that cursor for voice input. Donkey expands the cursor label into the compact input, records while Command is held, transcribes the speech into that input on release, submits it as a same-thread follow-up, and then lets the cursor retarget to the next action.
...
- Spawn display state is typed state. The model emits progress, target hints, and current selection; the overlay controller resolves hints against safe visible windows and owns Command-hold voice capture for the selected spawn; SwiftUI only renders the notch cue, cursors, labels, selection state, and compact inputs.
...
- Typed prompt and notch follow-up submissions show the spawn sequence: notch cue, desktop cursor emergence, direct travel to a known target or fallback holding point, typed progress label, retargeting when the target becomes visible, and fade-out for completed conversational turns.
- Stationary spawn labels expand on click/focus, accept compact multiline follow-up text, submit with Return or the send button, insert newlines with Shift-Return, collapse with Escape, and keep the cursor from moving while the draft is active.
- With multiple spawned cursors visible, clicking a cursor selects it; holding Command speaks to the selected/latest cursor, pauses movement during recording, shows transcription in that cursor's input, and resumes by submitting the transcript as a follow-up.
