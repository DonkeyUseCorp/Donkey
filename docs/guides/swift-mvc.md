...
- Keep controllers on `@MainActor` when they touch AppKit. Controllers may read `NSEvent.mouseLocation`, monitor double-Command activation, own `NSPanel`, and translate geometry into model state.
...
- the controller owns AppKit-only work such as the floating panel, double-Command activation, fixed-offset positioning, and movement timing
