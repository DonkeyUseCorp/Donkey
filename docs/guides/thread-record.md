...
Every session writes a running record of itself to `thread.md` — the real conversation plus the full reasoning and tool trace, one folder per task under Application Support (`Threads/<id>/thread.md`). It is human-readable markdown, appended live, so you can `tail -f` a run as it unfolds, and it is what the in-app conversation view renders back to the user. A compacted `summary.md` sits beside it.

**The one rule:** every session event a user (or a maintainer reading the run) would want to see goes into the thread, through the one transcript writer — never a bare `print`, never a log line that lives only in the console, never state that exists only in memory. If an event matters enough to happen, it matters enough to be in the record.

## How It Works

The thread file is opened once per task and only ever appended to:
...
The run loop records itself: the turn-trace manager is the single sink every model call and every executed step report to, so steps and model calls reach the thread without callers thinking about it. Events that happen *outside* a run — pausing, resuming, approving, or denying a task from the notch — are recorded by the overlay model's lifecycle announcer, which writes the same line to the in-app event store and to the thread. **The run loop records the work; the lifecycle announcer records the user's controls.**
...
| system event | a lifecycle event — understanding parsed, run finished, task paused, permission granted |
...
1. **One writer, append-only.** Open the thread once and append. The header is written a single time; later turns and events only add to the end. Never truncate or rewrite the file.
2. **Keyed by task id.** Each task owns its thread folder, addressed by task id, so events from one task never bleed into another.
...
4. **Plain text in the store, markdown in the thread.** The in-app event store keeps plain text — structured data the conversation view reads. The thread keeps the icon-prefixed markdown line. Presentation stays out of the data store.
