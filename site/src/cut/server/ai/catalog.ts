...
export const AI_TOOLS: AiToolDef[] = [
  ...
  },
  {
    name: "delete_item",
    description:
      "Delete a video clip (any track), soundtrack clip, or title by id. Deleting a track-0 clip ripples: its footprint closes and everything after it — clips, titles, captions, soundtrack — slides left in sync; items wholly inside that span are removed with it, straddlers are trimmed. Deletes on other tracks remove just that item.",
    inputSchema: obj({
...
export function attachedAssetsBlock(refs: unknown[]): string {
  ...
  const clipHint = refs.some((r) => (r as { scope?: string }).scope === "clip")
    ...
    : "";
  return `\n\n<attached_assets>\nThe user attached these assets to this message; their text may cite one by @handle or @name. Assets with scope "project" are in the open project (ids usable with the editor tools); "clip" assets are clips on the timeline — a video clip's id matches videoTrack in editor_state (a scene-run clip carries its sceneShot number there), an audio clip's (kind "audio") id matches soundtrack; "library" and "stock" assets live outside the project until imported; "file" assets came straight from the user's computer and exist only on this message:\n${JSON.stringify(refs)}${clipHint}\n</attached_assets>`;
}
