...
import { SKILLS, type DocType, type SkillKind } from "./data";
...
const DOC_DWELL: Record<DocType, number> = {
  sheet: 4200,
  word: 5400,
  pdf: 5000,
};
...
export function DonkeySkills() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      setDocType(
        (d) => DOC_ORDER[(DOC_ORDER.indexOf(d) + 1) % DOC_ORDER.length],
      );
    }, DOC_DWELL[docType]);
                  ...
                  {DOC_TABS.map(([k, label]) => (
                    <button
                      ...
                      className={cn(
                        ...
                        docType === k
                          ? "opacity-100 bg-coral"
                          : "opacity-50 hover:opacity-100 hover:bg-[#FCEAE3]",
                      )}
                      onClick={() => pickDoc(k)}
                    >
                      {label}
