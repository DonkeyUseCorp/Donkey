...
- Runtime shell: minimal run coordination, ordered events, bounded context assembly, local run artifacts, manual target context capture, local model runtime setup/registration in Application Support, in-memory reflex trace retention, and stage-split latency reports are supported.
...
3. Package and publish Donkey-compatible runtime downloads, then run real machine-local sidecar binaries registered by the app-managed installer for YOLO, Parakeet, and UI understanding, and record benchmark evidence for weather/media/document tasks.
