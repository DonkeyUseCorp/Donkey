...
The code now includes the first generic local-app task catalog, Ollama-compatible local-model command parser, deterministic fallback parser, app availability check, dry-run adapter boundary, local-app live runner, macOS launch/focus and keyboard-input backend, pointer-prompt typed command wiring, sample app-task definitions across weather/media/document domains, YOLO26 screenshot segmentation model-candidate metadata, and local voice-transcription model routing. It does not yet include Accessibility action execution, robust live control discovery, live local-model UI understanding over bounded screenshots/crops, live YOLO screenshot segmentation inference, live microphone transcription, semantic memory retrieval, remote-input redaction hooks, aggregate model observability, or provider-decoded memory write proposals.

## What Should Be Done Next

1. Manually verify the pointer-prompt-to-Weather path on a real macOS session: typed command submission, local-model intent parse or deterministic fallback, Weather launch/focus, guarded key/text execution, Accessibility visible-text observation, and final verification trace.
2. Add robust live control discovery and Accessibility actions for local-app workflows so tasks can find controls instead of relying only on keyboard shortcuts.
